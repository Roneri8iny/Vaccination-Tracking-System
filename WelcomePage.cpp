...
WelcomePage::WelcomePage(unordered_map<string, User>& userMap, vector<User>& fullyVaccinated, vector<User>& waitForFirstConfirm, vector<User>& waitForSecondConfirm, queue<User>& firstDoseQueue, queue<User>& seconDoseQueue, QStackedWidget& stackedWidget , QWidget* parent) 
	: QMainWindow(parent) /*, stackedWidget(stackedWidget)*/
{
	ui.setupUi(this);
...
void WelcomePage::savetofile(unordered_map<string, User>& userdata, queue<User>& first_dose_waiting_list, queue<User>& second_dose_waiting_list, vector<User>& full_vaccinated_users, vector<User>& wait_for_first_confirm, vector<User>& wait_for_second_confirm)
{
    ...
    if (users_took_one_dose.is_open())
    {
        while (!second_dose_waiting_list.empty())
        {
            ...
            users_took_one_dose << second_dose_waiting_list.front().getFullName() << "," << second_dose_waiting_list.front().getId() << "," << second_dose_waiting_list.front().getPassword() << "," << gender << "," << second_dose_waiting_list.front().getAge() << "," << second_dose_waiting_list.front().getAppointment() << ",\n";
    ...
    {
        while (!full_vaccinated_users.empty()) {

            for (int i = 0; i < full_vaccinated_users.size(); i++)
            {
    ...
    {
        while (!wait_for_first_confirm.empty()) {

            for (int i = 0; i < wait_for_first_confirm.size(); i++)
            {
    ...
    {
        while (!wait_for_second_confirm.empty()) {

            for (int i = 0; i < wait_for_second_confirm.size(); i++)
            {
