...
#include<QHeaderView>
viewWaitingList::viewWaitingList(queue<User>& waitingList, QStackedWidget& stackedWidget, QWidget* parent)
    : QMainWindow(parent)
{
    ui.setupUi(this);
...

{
    ...
    for (int i = 0; i < waitingListTable->rowCount(); i++)
    {
        ...
        for (int j = 0; j < waitingListTable->columnCount(); j++)
        {
            ...
            if (j == 5)
            {
                ////qstr = QDate::toString(userWaitingVector[i].getDate());
                //date = userWaitingVector[i].getDate();
                //qstr = date.toString("yyyy-MM-dd");
                //item->setText(qstr);
            }
            QFont font("Arial", 10);
            item->setFont(font);
