...
SignUp::SignUp(unordered_map<string, User>& userdata, vector<User>& fullyVaccinated,  vector<User>& waitForFirstConfirm, vector<User>& waitForSecondConfirm, queue<User>& firstDoseQueue, queue<User>& seconDoseQueue, QStackedWidget& stackedWidget, QWidget *parent)
    : QMainWindow(parent)
{
    ui.setupUi(this);
