...
AdminControls::AdminControls(unordered_map<string, User>& userMap, queue<User>& waitingList, queue<User>& firstDoseList, vector<User>& secondDoseList, QStackedWidget& stackedWidget, QWidget* parent)
	: QMainWindow(parent)
{
	ui.setupUi(this);
	...
	connect(ViewByAgeButton, &QPushButton::clicked, [this, &secondDoseList, &stackedWidget]()
		{

			onViewByAgeButtonClicked(ref(secondDoseList), ref(stackedWidget));
...
void AdminControls::onViewByAgeButtonClicked(vector<User>& secondDoseList, QStackedWidget& stackedWidget)
{
	stackedWidget.hide();

	ViewUserByAgeGUI* ViewUserByAge = new ViewUserByAgeGUI(secondDoseList, stackedWidget);
...
void AdminControls::onlogoutButtonClicked(unordered_map<string, User>& userMap, queue<User>& waitingList, queue<User>& firstDoseList, vector<User>& secondDoseList, QStackedWidget& stackedWidget)
{
	...
	stackedWidget.setCurrentIndex(0); //0
	stackedWidget.show();
}
