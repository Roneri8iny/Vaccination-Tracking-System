...
//User::User( string fullName,  string id,  string password,
//	bool male, bool female, int age,  string governorate,
//	bool isVaccinated, int doses,  QDate appointment) : fullName(fullName), id(id), password(password), male(male), female(female),
//	age(age), governorate(governorate), isVaccinated(isVaccinated),
//	doses(doses), appointment(appointment) {}
...
//void User::setAppointment(QDate appointment) {
//
//	this->appointment = appointment;
//}
...
QDate User::assignAppointment()
{
	...
	//QDate tomorrowDate = tomorrowDateTime.date();
	return tomorrowDate;
}
...
bool User::checkAppointmentDate(User user, vector<User>& waitForFirstConfirm, vector<User>& waitForSecondConfirm ,queue<User>& firstDoseQueue, queue<User>& secondDoseQueue)
{
	...
	if (user.doses == 0 && assignedDate <= currentdate) {
		...
		if (!waitForFirstConfirm.empty()) {
			...
			while (firstDoseQueue.front() != user) {    // crash 
